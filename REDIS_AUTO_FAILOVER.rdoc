...
* the redis-configurator polls the master from the redis-watcher every x-seconds, this is also the registration process for workers (they are added to the alive-servers)
* the redis-master sends a going_offline to the redis-watcher before a scheduled downtime
* when the redis-watcher finds the redis-master to be down he will do series of retries
* when the redis-master doesn't recover, the redis-watcher will call for a vote
