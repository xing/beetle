...
* subscribers should be able to determine the current redis-master without asking another
  process (as long as the redis servers are working and no reconfiguration is in progress)
...
* the rc-server promotes the former redis-slave to become the new redis-master (by sending SLAVEOF no one and setting the intended_role key to "master")
...
* the rc-server periodically tries to connect to the old master and configure it as a slave (by sending SLAVEOF new_master and setting intendend_role key to "slave")