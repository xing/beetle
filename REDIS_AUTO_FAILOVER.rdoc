...
* workers should be able to determine the current redis-master without asking another process (as long as the redis servers are working and no reconfiguration is in progress)
...
* on startup, an rc-client can determine the current redis-master without the help of the rc-server by asking *all* configured redis servers for their current role, making sure that no reconfiguration round is in progress (by checking that the intended_role key on all redis servers still matches their current role)
* if an rc-client cannot get the redis-master from the rc-server and cannot determine the redis-master itself, it will retry a couple of time before it finally exits
* periodically the rc-clients sends an "online" message to the rc-server, allowing it to keep a list of known rc-clients
* when the rc-server finds the redis-master to be down, it will retry a couple of times before starting the following reconfiguration round

===== when the redis-master doesn't recover (reconfiguration round)

* the rc-server sets the intended_role key on the redis-slave to "master"
...
* the rc-server sends a "new master" message containing the new redis-master to every rc-client
