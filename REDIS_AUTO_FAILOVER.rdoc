...
* the RCCs write the new redis-master to their redis master configuration file
...
See Beetle::Configuration for setting redis configuration server options.

=== On one server

Start the Redis Configuration Server:

  beetle configuration_server start -- --redis-servers redis-master:6379,redis-slave:6379 --client-ids rcc-1,rcc-2
...
On first worker server:
  ...
  beetle configuration_client start -- --redis-servers redis-master:6379,redis-slave:6379 --client-id rcc-1

On second worker server:

  beetle configuration_client start -- --redis-servers redis-master:6379,redis-slave:6379 --client-id rcc-2
