...
* if a worker cannot connect to the redis-coordinator or does not receive the redis-master info, it will retry a couple of time before it finally exits with an error message
* when the redis-coordinator finds the redis-master to be down he will retry a couple of times
