  ...
  class RedisConfigurationClient
    ...
    def daemon_controller
      @daemon_controller ||= DaemonController.new(
         ...
         :start_command => "ruby bin/beetle configuration_client start -- -v --multiple --redis-master-file #{redis_master_file} --id #{@name} --pid-dir #{tmp_path} --amqp-servers #{ENV['RABBITMQ_SERVERS'] || 'localhost:5672'}",
