  ...
  class RedisConfigurationClient
    ...
    def daemon_controller
      @daemon_controller ||= DaemonController.new(
         ...
         :start_command => "ruby bin/beetle configuration_client start -- -v --redis-servers #{@redis_servers.join(',')} --redis-master-file #{redis_master_file} --id #{@name} --pid-dir #{tmp_path} --amqp-servers 127.0.0.1:5672",
