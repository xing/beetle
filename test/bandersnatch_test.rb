...
class BandersnatchConfigTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:pub)
  end

  test "mode should be set if valid" do
    assert_equal :pub, @bs.mode
    bs = Bandersnatch::Base.new(:sub)
    assert_equal :sub, bs.mode
  end

  test "initializing should fail for invalid mode" do
    assert_raises(Bandersnatch::Error) { Bandersnatch::Base.new(:murks) }
  end
...
class BandersnatchSubscriberTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:sub)
  end
...
class BandersnatchSubscriberExchangeManagementTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:sub)
  end
...
class BandersnatchPublisherExchangeManagementTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:pub)
  end
...
class BandersnatchSubscriberQueueManagementTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:sub)
  end
...
class BandersnatchPublisherQueueManagementTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:pub)
  end
...
class BandersnatchSubscriptionTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:sub)
  end
  ...
  test "subscribe_message should subscribe with a subscription callback created from the registered block" do
    ...
    q = mock("QUEUE")
    q.expects(:subscribe).with({:ack => true, :key => "some_key.#"}).yields(header, Bandersnatch::Message.encode("data"))
  ...
  test "subscribe should fail if no handler exists for given message" do
    assert_raises(Bandersnatch::Error){ @bs.subscribe_message("some_message") }
  end
...
class BandersnatchServerManagementTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:sub)
  end
...
class BandersnatchPublisherPublishingTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:pub)
  end
  ...
  test "publishing should use the message ttl passed in the options hash to encode the message body" do
    data = "XXX"
    opts = {:ttl => 1.day}
    Bandersnatch::Message.expects(:encode).with(data, :ttl => 1.day)
...
class BandersnatchMessageIdInsertionTest < Test::Unit::TestCase
  def setup
    # Bandersnatch::Message.redis.flushdb
  end

  test "the database should not be checked if the message has no uuid" do
    body = Bandersnatch::Message.encode('my message', :with_uuid => false)
    message = Bandersnatch::Message.new("server", {}, body)
  ...
  test "inserting a messages uuid for the same queu into the database should fail the second time" do
    body = Bandersnatch::Message.encode('my message', :with_uuid => true)
    message = Bandersnatch::Message.new('server', {}, body)
...
class MessageDeDuplificationTest < Test::Unit::TestCase
  def setup
    @bs = Bandersnatch::Base.new(:sub)
  ...
  test 'a message handler should call the callback if the message id is not already in the database' do
    ...
    header = mock("header")
    header.expects(:ack)
    message = Bandersnatch::Message.new("server", header, body)

    Bandersnatch::Message.any_instance.expects(:insert_id).with(@queue).returns(true)
  ...
  test "a message handler should not call the callback if the message id is already in the database" do
    ...
    Bandersnatch::Message.any_instance.expects(:insert_id).with(@queue).returns(false)
    @callback.call(header, body)
  end
  ...
  test "the message callback should not get called and the message should not be ack'ed if the db check fails" do
    ...
    Bandersnatch::Message.any_instance.expects(:insert_id).raises(Exception)
  ...
  test "expired messages should be silently dropped without inserting a uuid into the database" do
    body = Bandersnatch::Message.encode("my message", :ttl => -1)
    header = mock("header")
    header.expects(:ack)
    message = Bandersnatch::Message.new("server", header, body)

    @handler.expects(:call).never
    Bandersnatch::Message.any_instance.expects(:insert_id).never
    @callback.call(header, body)
  end
end

class MessageEncodingTest < Test::Unit::TestCase
  test "a message should encode/decode the message format version correctly" do
    body = Bandersnatch::Message.encode("12345")
    header = mock("header")
    m = Bandersnatch::Message.new("server", header, body)
    assert_equal Bandersnatch::Message::FORMAT_VERSION, m.format_version
  end

  test "a message with uuid should have a uuid and the uuid flag set" do
    body = Bandersnatch::Message.encode("12345", :with_uuid => true)
    header = mock("header")
    m = Bandersnatch::Message.new("server", header, body)
    assert m.has_uuid?
    assert_equal(Bandersnatch::Message::FLAG_UUID, m.flags & Bandersnatch::Message::FLAG_UUID)
  end
  ...
  test "a message without uuid should have a nil uuid and the uuid flag not set" do
    ...
    m = Bandersnatch::Message.new("server", header, body)
    assert !m.has_uuid?
    assert_equal(0, m.flags & Bandersnatch::Message::FLAG_UUID)
  end

  test "encoding a message should set an expiration date" do
    Bandersnatch::Message.expects(:ttl_to_expiration_time).with(17).returns(42)
    body = Bandersnatch::Message.encode("12345", :ttl => 17)
    header = mock("header")
    m = Bandersnatch::Message.new("server", header, body)
  ...
  test "encoding a message should set the default expiration date if noe is provided in the call to encode" do
    Bandersnatch::Message.expects(:ttl_to_expiration_time).with(Bandersnatch::Message::DEFAULT_TTL).returns(42)
    body = Bandersnatch::Message.encode("12345")
    header = mock("header")
    m = Bandersnatch::Message.new("server", header, body)
    assert_equal 42, m.expires_at
